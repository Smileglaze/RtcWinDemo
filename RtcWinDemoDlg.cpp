...
BOOL CRtcWinDemoDlg::OnInitDialog()
{
	...
	// load login info in the last time.
	LoadProfile();
	...
	// reset registered remote window HWNDs
	memset(remote_window_hwnd_registered, 0, sizeof(remote_window_hwnd_registered));
	joined_room_ = false;
	...
	m_editRoomId.SetWindowText(StdStringToCString(room_id_));
	m_editUserId.SetWindowText(StdStringToCString(user_name_));
...
void CRtcWinDemoDlg::RemoveTrack(webrtc::VideoTrackInterface* track) {
	if (track && track->kind() == webrtc::MediaStreamTrackInterface::kVideoKind) {
		...
		if (it != map_of_video_renders_.end()) {
			CWnd* pWnd = NULL;
			pWnd = FromHandle(it->second->GetWindow());
			if (pWnd != nullptr) {
				int ctrlID = pWnd->GetDlgCtrlID();
				remote_window_hwnd_registered[ctrlID - IDC_REMOTE_WIN_1] = 0;
			}
...
HWND CRtcWinDemoDlg::AllocateWindow(bool is_local, std::string track_id) {
	...
	if (is_local) {
		...
		for (int i = 0; i < 3; i++)
		{
			if (remote_window_hwnd_registered[i] == 1) {
				// if this window HWND has been registered
				continue;
			}
			// IDC_REMOTE_WIN_1 to IDC_REMOTE_WIN_3
			pWnd = GetDlgItem(IDC_REMOTE_WIN_1 + i);
			// register
			remote_window_hwnd_registered[i] = 1;
			break;
		}
	}
...
void CRtcWinDemoDlg::OnBnClickedBtnSend()
{
	if (joined_room_) {
		CString strMsg;
		m_editMessage.GetWindowText(strMsg);

		if (strMsg.IsEmpty()) {
			AfxMessageBox(_T("No Message to Be Sent"));
			return;
		}
		...
		cJSON_AddItemToObject(info_json, "msg", cJSON_CreateString(CStringToStdString(strMsg).c_str()));
		...
		char* notification = cJSON_Print(root_json);

		LogPrintf(notification);
		if (websocket_ && notification) {
			websocket_->sendMessage(std::make_shared<std::string>(notification));
			/* Display on local screen. However, "broadcast" will also notify the local user.
			*  So there is no need to do this.
			*/
			//OnWebSocketNotify(ConvertMethodToCommand("broadcast"), root_json);
		}
...
void CRtcWinDemoDlg::SaveProfile() {
	WritePrivateProfileString(_T("LoginInfo"),
							  _T("room_name"),
							  StdStringToCString(room_id_),
							  _T(".\\LoginInfo.ini"));
	WritePrivateProfileString(_T("LoginInfo"),
							  _T("user_name"),
							  StdStringToCString(user_name_),
							  _T(".\\LoginInfo.ini"));
}

void CRtcWinDemoDlg::LoadProfile() {
	CString strPath = _T(".\\LoginInfo.ini");
	CFileStatus status;

	if (CFile::GetStatus(strPath, status))
	{
		// Frome  ULONGLONG to DWORD , the file size should be smaller than 4GB
		DWORD dFileSize = (DWORD)status.m_size;

		CString strUserName, strRoomName;

		GetPrivateProfileString(_T("LoginInfo"),
								_T("room_name"),
								_T("Error: failed"),
								strRoomName.GetBuffer(dFileSize),
								dFileSize,
								strPath);
		...
		GetPrivateProfileString(_T("LoginInfo"),
								_T("user_name"),
								_T("Error: failed"),
								strUserName.GetBuffer(dFileSize),
								dFileSize,
								strPath);
		strUserName.ReleaseBuffer();

		user_name_ = CStringToStdString(strUserName);
		room_id_   = CStringToStdString(strRoomName);;
	}
}
...
void CRtcWinDemoDlg::AppendMessage(CString strMsg, bool title) {
	...
	// whether sent by the local user.
	bool isLocal = user_name_.compare(CStringToStdString(strMsg)) == 0;
	
	cf.cbSize      = sizeof(cf);
	cf.dwMask      = CFM_BOLD | CFM_COLOR | CFM_BACKCOLOR | CFM_SIZE | CFM_OFFSET;
	cf.dwEffects   = title ?  CFE_BOLD : 
							  isLocal ? CFE_SHADOW : 0;
	cf.crTextColor = title ? isLocal ? RGB(0x1E, 0x90, 0xFF) : 
									   RGB(0xA5, 0x2A, 0x2A) : 
							 RGB(0, 0, 0);
	cf.crBackColor = title ? isLocal ? RGB(0xE5, 0xEB, 0xFF) :
									   RGB(0xF5, 0xDC, 0xD8) :
							 RGB(0xFF, 0xFF, 0xFF);
	cf.yHeight	   = 10 * 20; // font size: 1/20 of a printer's point
	cf.yOffset     = 5;

	// add title format
	title ? 
		isLocal ? strMsg.Format(_T(" ◀ %s:\n"), strMsg.GetString()) :
			      strMsg.Format(_T(" ▶ %s:\n"), strMsg.GetString()) :
		// add indent to the message
		strMsg.Format(_T("     %s"), strMsg.GetString());
